...
class Six_Growth_Engine {
    ...
    public static function on_abandon( $user_id, $step, $score ) {
        ...
        error_log("6ix on_abandon: uid={$user_id} step={$step} score={$score}");

        // Cooldown: once per 24 hours per user — prevents repeat sends from multiple crons
        $last = intval( get_user_meta( $user_id, 'six_last_abandon_trigger', true ) );
        if ( $last && ( time() - $last ) < 86400 ) {
            error_log("6ix on_abandon: 24h cooldown active uid={$user_id} last=" . date('H:i:s',$last) . " — skip");
            return;
        }
        ...
        // ── Odoo: move lead to Abandoned stage ───────────────────────────────
        if ( class_exists('Six_Odoo') ) {
        ...
        if ( ! empty($lead_id) && class_exists('Six_Odoo') ) {
            try {
                ...
                $step_labels = array(
                    0 => 'Login / Email entry', 1 => 'Account Creation',
                    2 => 'Service Selection',   3 => 'Questionnaire',
                    4 => 'AI Strategy Review',  5 => 'Agreement & Payment',
                );
                $completed = array();
                for ($i = 1; $i < $step; $i++)
                    $completed[] = "  ✓ Step {$i}: " . ($step_labels[$i] ?? "Step {$i}");
                ...
                $ctx = array();
                if (!empty($co->business_name)) $ctx[] = "Business: {$co->business_name}";
                if (!empty($co->industry))       $ctx[] = "Industry: {$co->industry}";
                if (!empty($co->goal))           $ctx[] = "Goals: {$co->goal}";
                if (!empty($co->platforms))      $ctx[] = "Services: " . str_replace(',',', ',$co->platforms);
                if (!empty($co->mktg_budget))    $ctx[] = "Budget: \${$co->mktg_budget}/mo";
                if (!empty($co->competitors))    $ctx[] = "Competitors: {$co->competitors}";
                if (!empty($co->ads_keywords))   $ctx[] = "[Google Ads] Keywords: {$co->ads_keywords}";
                if (!empty($co->seo_keywords))   $ctx[] = "[SEO] Keywords: {$co->seo_keywords}";
                if (!empty($co->gbp_name))       $ctx[] = "[GBP] Name: {$co->gbp_name}";
                if (!empty($co->web_goal))       $ctx[] = "[Website] Goal: {$co->web_goal}";
...
"
                       . "ABANDONED AT
  Step {$step}/5 — " . ($step_labels[$step] ?? "Step {$step}") . "

"
                       . "COMPLETED STEPS
" . (empty($completed) ? '  None' : implode("
",$completed)) . "

"
                       . "DATA COLLECTED
" . (empty($ctx) ? '  None yet' : implode("
",$ctx)) . "
...
"
                ...
                Six_Odoo::create_activity(
                    $lead_id, "Abandoned Onboarding — Follow Up Required",
                    $note, 'Todo', 0, $advisor_uid
                );
                error_log("6ix on_abandon: activity created lead={$lead_id}");
            } catch (\Throwable $e) {
                error_log("6ix on_abandon: activity exception: " . $e->getMessage());
            }
        }
        ...
        error_log("6ix on_abandon: complete uid={$user_id}");
    }
    /**
     * RE-ENGAGEMENT FLOW.
